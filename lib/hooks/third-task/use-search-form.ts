...
export function useSearchForm({ handleSubmit, dropdownRef, countries }: { handleSubmit: (selectedItem: GeoEntity | null) => void | Promise<void>, dropdownRef: React.RefObject<DropdownRef | null>, countries: GeoEntity[] }) {
    ...
    // the diff between first task and third task is that we need to pass countries to the useSearch hook
    // because in third task we need to show country flag in the hotel card
    const { result: items } = useSearch({ query: search, countries: countries || [], type: type || 'country' });
